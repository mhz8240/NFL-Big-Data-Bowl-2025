...
def plot_distance_projection_ratio(tracking_combined_filtered,temp_df2,plays_combined):
  ...
  #plot details
  plt.title('EPA/Play Allowed Based on Distance Projection Ratio Score By Team (Plays With Motion)')
  plt.xlabel('Dist-PR (Distance Projection Ratio)')
  plt.ylabel('Average EPA Allowed')
  for i in range(32):
      plt.text(seaborn_projection_ratio.projection_ratio[i] + 0.005, seaborn_projection_ratio.epa_allowed[i], seaborn_projection_ratio.index[i], fontsize=8)
...
def plot_disp_projection_ratio(tracking_combined_displacement, temp_df,plays_combined):
  ...
  #add plot details
  #plot details
  plt.title('EPA/Play Allowed Based on Displacement Projection Ratio Score By Team (Plays With Motion)')
  plt.xlabel('Disp-PR (Displacement Projection Ratio)')
  plt.ylabel('Average EPA Allowed')
  for i in range(32):
...
def plot_bargraph_nested(values):
  ...
  labels_shortened = ['Top 5','Bottom 5','Top 10','Bottom 10']
  width = 0.3
  ...
  ax.set_title('Average EPA/Play allowed based on Total Reaction Score Ranking')
  # ax.legend()
...
def plot_bargraph(x_ratios,plays_combined):
  ...
  values=[total_topfive_mean,total_botfive_mean,total_topten_mean,total_botten_mean,]
  # plt.figure()
